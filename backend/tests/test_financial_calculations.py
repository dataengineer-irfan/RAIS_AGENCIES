...
def test_invoice_line_calculation_without_discount():
    # 4 packets of Hup Hup French Fries @ Rs.310 with 5% GST
    items = [{
    ...
    assert res["tax_amount"] == Decimal("62.00") # 1240 * 0.05
    assert res["total_amount"] == Decimal("1302.00")

def test_invoice_line_calculation_with_line_and_invoice_discounts():
    # 2 packets of Milky Mist Mozzarella @ Rs.920 with 10% line discount and 12% GST
    ...
    # Line Taxable: 1840 - 184 = 1656.00
    # Line Tax: 1656 * 0.12 = 198.72
    # Total Disc: 184 + 40 = 224.00
    # Invoice Taxable: 1840 - 224 = 1616.00
    # Total: 1616 + 198.72 = 1814.72
    assert res["subtotal"] == Decimal("1840.00")
    assert res["discount_amount"] == Decimal("224.00")
    assert res["taxable_amount"] == Decimal("1616.00")
    assert res["tax_amount"] == Decimal("198.72")
    assert res["total_amount"] == Decimal("1814.72")
